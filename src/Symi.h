	...
	*/
	...
	class Symi : public Magi<order, step>
	{
		public:
			...
			Symi(double T, double deltaT, int skipper) : Magi<order,step>(T,deltaT,0,skipper)
			{
				MakeName();
			}
		...
		Quaternion virtual Magnus(double duration)
		{
			...
			if (Symi::Order == 0 || abs(Symi::L.Vector(2)) < 1e-14)
			{
				return duration * InvMult(Symi::J,Symi::L)/2;
			}
			...
			if (Symi::Order > 1)
			{
				...
				double m2z = lParallel*lParallel/(zeta*zeta * Symi::J[1]*Symi::J[1]) * (sin(zeta*duration) - zeta * duration );
